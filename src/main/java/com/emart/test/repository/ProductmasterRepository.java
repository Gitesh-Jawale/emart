...
public interface ProductmasterRepository extends JpaRepository<Productmaster,Integer>{
	...
	@Query(nativeQuery=true, value="select p.productid,p.productname,pt.configdtl from productmaster p inner join productdtlmaster pt on p.productid=pt.productid where p.productid=:productId")
	List<Object[]> getsample(@Param("productId") Integer productId);
